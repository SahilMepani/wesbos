...
What we have done is turned our HTML into true elements, that are still not on the page anywhere, but we have them as DOM elements and can do things like look for an image inside of a fragment using the following code 👇
...
You could also use `insertAdjacentElement()` which we learned about previously.
